...
- Policy `percentage-v2`: zero to two entries per Berlin day, at most two open positions, no leverage. Each entry allocates **10% of EUR cash available immediately before that entry, including the entry fee**. Its amount compounds with realized results; it never uses a hardcoded account value, reserved funds or the EUR valuation of imported crypto as buying cash. Planned loss remains capped at 0.25% of strategy equity, position notional at 10%, aggregate exposure at 20%.
- Stop at 2% below entry fill price; target at 4% above entry fill price, before exit fees and slippage. Existing positions retain their stored stop/target. Exit on an hourly close below EMA20 or after 48 hours. Exits run before entry filters at the next fresh polled bid. Stops are not resting orders or guaranteed prices. ATR remains only an entry volatility/chasing filter.
- Round quantity down to the public instrument step and respect its published minimum size/notional. No extra fixed EUR 1 floor is imposed by this component. If the percentage budget is too small, wait rather than round the order up or increase allocation.
- The report separately values current ledger totals, available assets and reserves at fresh public bids before costs. Missing prices produce `null`, never an invented zero. This informational valuation neither releases reserves nor changes the opening capital/performance basis. `entryReadiness` reports the current cash allocation fraction and budget.
- Educational costs are 0.09% fees and 0.05% adverse slippage per side, not statements of actual broker charges.
- Reduce allocation to 5% of available EUR and risk to 0.125% after two consecutive losses or 2% drawdown; restore the base tier once neither reduction condition holds. Pause new risk at 1% daily loss, 3% weekly loss or 5% drawdown. Gates are re-evaluated every cycle. The owner toggle pauses entries only; exits continue.
