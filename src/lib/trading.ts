...
function validateContext(d:Draft,c:TradingContext,sessionId?:string){
 ...
 return {accountId:d.accountId,symbol:d.symbol,side:d.side,type:d.type,quantity:d.quantity,price:String(price),limitPrice:d.limitPrice??null,timeInForce:d.type==='market'?'IOC':'GTC',estimatedNotional:notional.toFixed(8),estimatedFee:fee.toFixed(8),estimatedTotal:(d.side==='buy'?notional.add(fee):notional.sub(fee)).toFixed(8),currency:quote,feeRate:'0.09%',feeNote:'تقدير محافظ للتنفيذ الآخذ للسيولة؛ الرسوم النهائية من سجل الوسيط.',priceSource:c.quote.source,quoteAt:c.quote.updatedAt,warning:d.type==='market'?'السعر والتكلفة تقديريان وقد يتغير سعر التنفيذ.':'قد ينفذ الأمر جزئياً أو يبقى مفتوحاً حتى التنفيذ أو الإلغاء في منصة الوسيط.',mode:d.mode};
}
