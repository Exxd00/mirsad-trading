...
function validateContext(d:Draft,c:TradingContext,sessionId?:string){
 ...
 return {accountId:d.accountId,symbol:d.symbol,side:d.side,type:d.type,quantity:d.quantity,price:String(price),limitPrice:d.limitPrice??null,timeInForce:d.type==='market'?'افتراضي الوسيط':'GTC',estimatedNotional:notional.toFixed(8),estimatedFee:fee.toFixed(8),estimatedTotal:(d.side==='buy'?notional.add(fee):notional.sub(fee)).toFixed(8),currency:quote,feeRate:'0.09%',feeNote:'تقدير افتراضي لأغراض التعلّم فقط؛ لا توجد رسوم أو قيمة مالية حقيقية.',priceSource:c.quote.source,quoteAt:c.quote.updatedAt,warning:d.type==='market'?'السعر والتكلفة افتراضيان لأغراض المحاكاة التعليمية فقط.':'هذا أمر تجريبي افتراضي بلا قيمة مالية ولا يصل إلى وسيط حقيقي.',mode:d.mode};
}
